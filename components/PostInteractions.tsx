...
const PostInteractions = ({count, isLiked, isReposted, isSaved, comment}: {
  ...
  return (
    <div className={`flex pt-8 gap-16  ${comment ? "justify-between" : "justify-end mr-10"}`}>
       <div className="flex gap-1 group cursor-pointer">
        <Heart size={27} className={`text-foreground group-hover:text-red-500 ${isLiked ? "text-red-500" : ""}`} />
        <span className="text-foreground/70 ml-2 self-center">{likes}</span>
       </div>
       <div className="flex gap-1 group cursor-pointer">
        <Repeat2 size={27} className={`text-foreground group-hover:text-green-500 ${isReposted ? "text-green-500" : ""}`} />
       ...
       <Bookmark size={27} className={`text-foreground hover:text-blue-500 ${isSaved ? "text-blue-500" : ""}`} />
     </div>
  )
}
